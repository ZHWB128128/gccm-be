...
def test_covariant_hessian_reduces_to_euclidean_on_flat_metric():
    """平坦度量（Γ=0）下协变 Hessian 应与欧氏 Hessian 相同。"""
    from gccm_be.geometry.curvature import CurvatureAnalyzer
    from gccm_be.types import ControlInput, ExternalInput

    ls = make_landscape(coupling=0.0, alpha=0.0)
    state = SystemState([27.0, 28.0], list(ls.manifold.labels))
    ctrl = ControlInput([-2.0], ["Q_hvac"])
    ext = ExternalInput([32.0, 0.2, 0.5, 1.0], ["T_out", "solar", "occ", "price"])

    euclid = CurvatureAnalyzer(covariant=False).analyze(ls, state, ctrl, ext)
    covar = CurvatureAnalyzer(covariant=True).analyze(ls, state, ctrl, ext)
    # flat metric -> Christoffel = 0 -> covariant correction = 0
    assert np.max(np.abs(euclid.hessian - covar.hessian)) < 1e-6
    assert covar.covariant is True
    assert euclid.covariant is False


def test_covariant_hessian_differs_on_curved_metric():
    """强状态相关/耦合度量下协变 Hessian 应偏离欧氏 Hessian（Γ≠0 生效）。"""
    from gccm_be.geometry.curvature import CurvatureAnalyzer
    from gccm_be.types import ControlInput, ExternalInput

    ls = make_landscape(coupling=0.8, alpha=0.6)
    # away from setpoint so ∂E ≠ 0 and metric is strongly state-dependent
    state = SystemState([30.0, 31.0], list(ls.manifold.labels))
    ctrl = ControlInput([-3.0], ["Q_hvac"])
    ext = ExternalInput([34.0, 0.3, 0.6, 1.5], ["T_out", "solar", "occ", "price"])

    euclid = CurvatureAnalyzer(covariant=False).analyze(ls, state, ctrl, ext)
    covar = CurvatureAnalyzer(covariant=True).analyze(ls, state, ctrl, ext)
    # the covariant correction −Γ^k_ij ∂_k E must be non-trivial here
    assert np.max(np.abs(euclid.hessian - covar.hessian)) > 1e-6
