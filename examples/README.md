...
| `two_zone_config_loop.py` | 双区域闭环（纯 JSON 配置切换模型，读 `config_two_zone.json`） |
...
| `rg_consistency_demo.py` | RG 跨尺度动力学一致性检验（C 随失衡度） |
| `rg_consistency_observable.py` | 均值 vs 峰值可观测的粗粒化一致性对比（序参量加权的几何必要性） |

## BOPTEST 标准试验台（在 Linux 设备 /opt/boptest 上执行）

| 脚本 | 说明 |
|---|---|
| `boptest_direct.py` | bestest_air FMU 直连：GCCM(辨识+预测) vs 规则基线 24h 对比 |
| `boptest_abab_drill.py` | ABAB M&V 演练（PILOT_PLAN §5 协议：四期交替+配对对比） |
| `boptest_cw_sweep.py` | comfort_weight 参数扫描（硬约束开关对照） |
| `boptest_twozone_drill.py` | twozone_apartment_hydronic 双区演练（可选 --identify/--with-slab） |
| `boptest_mz_air_drill.py` | multizone_office_simple_air Sou/Nor 两区演练 |

原始结果数据归档见 [docs/data/](data/README.md)。

## 试点接入

| 脚本 | 说明 |
|---|---|
| `ha_pilot_loop.py` | 实楼试点闭环：Home Assistant 读数 → GCCM 决策 → 写回空调设定温度（`--once` 配 cron 单步调度）；需真实 HA 环境与 `HA_TOKEN` |
