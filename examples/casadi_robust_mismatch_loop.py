...
from gccm_be.geometry.casadi_robust_solver import CasadiRobustGeodesicSolver
...
def main() -> None:
    ...
    solver = CasadiRobustGeodesicSolver(
        nominal_sim,
        landscape,
        [pessimistic_sim],
        horizon=HORIZON,
    ...
    for i in range(STEPS):
        ...
        if not traj.controls:
            print(f"step {i}: solver failed, skip")
            t += STEP_H
            continue
