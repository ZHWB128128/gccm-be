...
def _casadi_step(building, x, u, w, dt):
    ...
    if isinstance(building, TwoZoneRCBuildingModel):
        with_slab = bool(getattr(building, "with_slab", False))
        T_air_A, T_wall_A, T_air_B, T_wall_B, T_partition = x[0], x[1], x[2], x[3], x[4]
        Q_A, Q_B = u[0], u[1]
        if with_slab:
            T_slab_A, T_slab_B = x[5], x[6]
            heat_A = (T_slab_A - T_air_A) / building.r_slab
            heat_B = (T_slab_B - T_air_B) / building.r_slab
        else:
        ...
        if with_slab:
            dT_slab_A = (Q_A - heat_A) / building.c_slab
            dT_slab_B = (Q_B - heat_B) / building.c_slab
            return x + dt * ca.vertcat(dT_air_A, dT_wall_A, dT_air_B, dT_wall_B,
                                       dT_partition, dT_slab_A, dT_slab_B)
...
class CasadiGeodesicSolver:
    ...
    def solve(
        ...
        def comfort_expr(value: ca.MX, label: str, setpoint: float, scale: float) -> ca.MX:
            ...
            bounds = self.landscape.bounds_for(label)
            if bounds is not None:
                lo, hi = bounds
                above = ca.fmax(0.0, value - hi)
                below = ca.fmax(0.0, lo - value) * below_penalty
                excess = above + below
            else:
        ...
        for k in range(self.horizon):
            ...
            for i, lab in enumerate(self.landscape.manifold.labels):
                if lab in setpoints:
                    ...
                    comfort += comfort_expr(xk[i], lab, setpoints[lab], scale)
            step_cost += weights.get("comfort", 1.0) * comfort

            # 电费（与 scipy 后端一致：部分负荷 COP，每台容量按 unit_bounds 取）
            elec = 0.0
            unit_caps = [max(abs(lo), abs(hi), 1e-9) for lo, hi in hvac.bounds()]
            for j in range(n_u):
                q = u[j]
            ...
            # 硬舒适约束：每个空气温度状态（每区 bounds_for，缺省回退标量）
            for idx in air_indices:
                lab = self.landscape.manifold.labels[idx]
                bnds = self.landscape.bounds_for(lab)
                if bnds is not None:
                    lo_b, hi_b = bnds
                    opti.subject_to(xk_next[idx] <= hi_b)
                    opti.subject_to(xk_next[idx] >= lo_b)
        ...
        for i, lab in enumerate(self.landscape.manifold.labels):
            if lab in setpoints:
                scale = self.landscape.manifold.scale.get(lab, 1.0)
                terminal += comfort_expr(X[-1][i], lab, setpoints[lab], scale)
